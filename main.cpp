...
public:
    ...
    bool Init()
    {
        ...
        if (!m_quad.LoadMesh("C:/Content/quad.obj")) {
            return false;
        }

        m_pGroundTex = new Texture(GL_TEXTURE_2D, "C:/Content/grey.jpg");

        if (!m_pGroundTex->Load()) {
            return false;
        }

        if (!m_mesh.LoadMesh("C:/Content/hheli.obj")) {
            return false;
        }
...
int main(int argc, char** argv)
{
    ...
    if (!GLUTBackendCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, false, "Lab 6 Tutorial 42")) {
        return 1;
    }
